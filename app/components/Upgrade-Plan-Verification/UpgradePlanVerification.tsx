...
const UpgradePlanVerification = () => {
  const searchParams = useSearchParams();
  const email = searchParams.get('email');
  const subscriptionSlug = searchParams.get('planSlug');
  const otpLength = 4;
  ...
  const otpVerificationClick = async () => {
    ...
    if (upgradePlanVerificationValue) {
      ...
      const payload: PlanVerificationFormValues = {
        ...
        email: email as string,
        otp_type: 'plan_upgrade',
        new_plan_uuid: subscriptionSlug as string
      };

      try {
        const response = await dispatch(upgradePlanVerification(payload)).unwrap();
...
export default UpgradePlanVerification;
